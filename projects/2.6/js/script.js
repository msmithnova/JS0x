...
var level;
var totalGood, totalWrong, score;
...
function startGame(nb) {
    ...
    if (!gameStarted) {
        ...
        initialNumberOfBalls = 10;
        document.querySelector("#ballSpeed").value = globalSpeedMutiplier;
        document.querySelector("#nbBalls").value = initialNumberOfBalls;
        totalGood = 0;
        totalWrong = 0;
        score = 0;
    }
...
function drawBallNumbers(balls) {
    ...
    if(balls.length === 0) {
        ...
        ctx.fillText("Press space bar to start new game.", 20, h/2);
        drawOverallStats();
    } else if(goodBallsEaten === numberOfGoodBalls) {
        //ctx.fillText("You Win! Final score : " + (initialNumberOfBalls - wrongBallsEaten), 20, 30);
        level++;
        totalGood += goodBallsEaten;
        totalWrong += wrongBallsEaten;
        score += (goodBallsEaten * initialNumberOfBalls - wrongBallsEaten) * level;
        globalSpeedMutiplier += 0.1;
        document.querySelector("#ballSpeed").value = globalSpeedMutiplier;
        nb = ++initialNumberOfBalls;
        document.querySelector("#nbBalls").value = nb;
        changeNbBalls(nb);
    } else {
...
function testCollisionWithPlayer(b, index) {
    if(circRectsOverlap(player.x, player.y, player.width, player.height,
                    b.x, b.y, b.radius)) {
...
function drawOverallStats() {
    ctx.fillText("Level: " + level, 5, h - 80);
    ctx.fillText("Total Good Balls eaten: " + totalGood, 5, h - 60);
    ctx.fillText("Total Wrong Balls eaten: " + totalWrong, 5, h - 40);
    ctx.fillText("Score: " + score, 5, h - 20);
}

function keyPressed(evt) {
    if (gameStarted == false && evt.key == " ") {
        startGame(10);
    }
}